...
function validateAndEnrichName(name, category, location, source, description) {
    ...
    // Enhance artist-only names for galleries (add "Exhibition")
    if (category === 'art' && source && /gallery|museum/i.test(source)) {
        // If name is just 1-2 words and doesn't contain descriptive terms
        if (words.length <= 2 && !/exhibition|show|opening|retrospective|collection|works|presents|festival|night/i.test(cleanName)) {
