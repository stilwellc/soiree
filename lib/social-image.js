...
const CARD_COLORS = {
  ...
  'Culture & Community': '#D4E0D8',
  'New York · Friday': '#E0DDE8',
  'New York · Saturday': '#DDE5E8',
  'New York · Sunday': '#E8E0D8',
  'Hoboken & JC · Friday': '#DDE8DF',
  'Hoboken & JC · Saturday': '#E8DDD8',
  'Hoboken & JC · Sunday': '#E0E5DD',
};
...
// Abbreviate long location names for better card readability
function abbreviateLocation(location) {
  ...
  if (abbreviations[location]) return abbreviations[location];

  // Check if location contains these phrases
 ...
 * @param {string} [options.metaField] - 'date' (default) or 'location' for right-side meta
 * @returns {Promise<Buffer>} PNG image buffer (1080x1350)
 */
async function generateSocialCard(categoryName, events, weekLabel, totalWeekEvents, options = {}) {
  ...
  const metaField = options.metaField || 'date';

  // Build event rows
  const eventRows = displayEvents.map((event, i) => {
    const meta = metaField === 'location' ? abbreviateLocation(event.location || '') : formatShortDate(event.start_date);
    return {
      type: 'div',
      props: {
        style: {
